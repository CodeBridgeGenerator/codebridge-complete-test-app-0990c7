...
import objext121FakerFactory from "./objext121FakerFactory";
...
const Objext121FakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = objext121FakerFactory(props.user,fakerCount,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("objext121").create(elem));
...
export default connect(mapState, mapDispatch)(Objext121FakerDialogComponent);
