...
import allfieldsFakerFactory from "./allfieldsFakerFactory";
...
const AllfieldsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = allfieldsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("allfields").create(elem));
...
export default connect(mapState, mapDispatch)(AllfieldsFakerDialogComponent);
