...
import arrayfieldsFakerFactory from "./arrayfieldsFakerFactory";
...
const ArrayfieldsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = arrayfieldsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("arrayfields").create(elem));
...
export default connect(mapState, mapDispatch)(ArrayfieldsFakerDialogComponent);
